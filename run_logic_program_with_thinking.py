...
# -*- coding: utf-8 -*-
"""
便捷运行脚本 - 用于运行 models/logic_program_with_thinking.py
支持选择数据集、模型和API提供商
此版本会在输出的 logic_programs 中添加 "thinking" 标签保存思考过程
"""
...
DATASET_SPLIT = '5'      # 可选: 'dev', 'test', 'train'

# API配置
API_PROVIDER = 'iflow'     # 可选: 'openai', 'zhipuai', 'iflow'
MODEL_NAME = 'glm-4.6'  # iflow模型: 'TBStars2-200B-A13B'，'deepseek-v3.2'
                                   # 智谱AI模型: 'glm-4.6', 'glm-4.5', 'glm-4-flash-250414', 'glm-3-turbo'
                                   # OpenAI模型: 'gpt-4', 'gpt-3.5-turbo', 'gpt-4-turbo', 'text-davinci-003'

# 其他配置（一般不需要修改）
DATA_PATH = './data'
SAVE_PATH = './outputs/logic_programs_with_thinking'
MAX_NEW_TOKENS = 100000  # 增加到 20000 以避免输出被截断
STOP_WORDS = '------'
BATCH_SIZE = 1
MAX_CONCURRENT = 1
MAX_RETRIES = 1
...
# 导入 logic_program_with_thinking 模块
try:
    from models.logic_program_with_thinking import LogicProgramGeneratorWithThinking
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='便捷运行脚本 - 用于运行 models/logic_program_with_thinking.py\n\n'
...
示例用法:
  ...
  python run_logic_program_with_thinking.py

  # 方式2: 使用命令行参数覆盖配置
  python run_logic_program_with_thinking.py --dataset_name ProntoQA --model_name glm-4 --api_provider zhipuai

  # 方式3: 只覆盖部分参数
  python run_logic_program_with_thinking.py --dataset_name FOLIO --split test

  # 使用OpenAI API
  python run_logic_program_with_thinking.py --api_provider openai --model_name gpt-4
...
def main():
    """主函数"""
    print("=" * 60)
    print("Logic Program Generator With Thinking - 便捷运行脚本")
    print("=" * 60)
    print()
    ...
    print(f"  注意: 此版本会在输出中添加 'thinking' 字段保存思考过程")
    print()
    ...
    try:
        ...
        print(f"注意: 输出文件包含 'thinking' 字段，保存了模型的思考过程")
