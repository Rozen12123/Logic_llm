...
MODEL_NAME = "Qwen3-8B"                  # 模型名称（根据实际部署的模型修改）

STOP_WORDS = '------'
MAX_NEW_TOKENS = 20000
TEMPERATURE = 0.0
MAX_CONCURRENT = 50           # 控制并发 API 调用
BATCH_SIZE = 50               # 每次批量请求的样本数
...
try:
    ...
    from models.utils import OpenAIModel  # 兜底
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description='让大模型直接阅读 outputs/logic_programs 中的符号程序，并生成 inference 结果文件（使用 vLLM 后端）',
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument('--dataset_name', type=str, default=DATASET_NAME, choices=SUPPORTED_DATASETS)
    parser.add_argument('--split', type=str, default=DATASET_SPLIT, choices=['dev', 'test', 'train'])
    ...
    parser.add_argument('--output_suffix', type=str, default=OUTPUT_SUFFIX,
                        help='输出文件名追加的后缀')
    parser.add_argument('--api_url', type=str, default=VLLM_API_URL,
                        help='vLLM API URL (通过 Nginx)')
    parser.add_argument('--api_key', type=str, default=VLLM_API_KEY,
                        help='vLLM API Key')
...
def main():
    ...
    print("=" * 60)
    print("LLM Symbolic Inference Runner - vLLM 版本")
    print("=" * 60)
    print(f"数据集: {args.dataset_name} ({args.split})")
    print(f"模型: vLLM / {args.model_name}")
